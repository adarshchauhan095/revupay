...
class UserDashboardView extends GetView<UserController> {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              _statsRow(context),
              const SizedBox(height: 16),
              Text('Active Campaigns', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
  ...
  Widget _welcomeCard(BuildContext ctx) => Obx(() => Card(
    child: ListTile(
      ...
      subtitle: Text('Wallet: ₹${controller.currentUser.value?.wallet?.toStringAsFixed(2) ?? '0.00'}',
          style: Theme.of(ctx).textTheme.bodyMedium?.copyWith(color: Theme.of(ctx).colorScheme.primary)),
  ...
  Widget _statsRow(BuildContext ctx) => Obx(() {
    ...
    final approved = controller.reviews.where((r) => r['status'] == 'approved').length;
    final pending = controller.reviews.where((r) => r['status'] == 'pending').length;
    final earnings = controller.reviews.where((r) => r['status'] == 'approved')
    ...
    return Row(
      children: [
        ...
        const SizedBox(width: 8),
        _statCard(ctx, 'Earnings', '₹${earnings.toStringAsFixed(0)}', Icons.monetization_on),
      ],
    );
  });

  Widget _statCard(BuildContext ctx, String title, String value, IconData icon) => Expanded(
    child: Card(
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            Text(value, style: Theme.of(ctx).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
            Text(title, style: Theme.of(ctx).textTheme.bodySmall?.copyWith(
                color: Theme.of(ctx).colorScheme.onSurface.withOpacity(.7)), textAlign: TextAlign.center),
          ],
        ),
      ),
    ),
  ...
  Widget _campaignList() => Obx(() {
    ...
    if (controller.campaigns.isEmpty) return const Center(child: Text('No active campaigns'));
